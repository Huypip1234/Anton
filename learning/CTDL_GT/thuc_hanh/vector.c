...
// initzation O(1)
void vecInit(struct Vector *vec, int initCapacity) {
	(*vec).size = 0; 
	(*vec).capacity = initCapacity; //default = 16 bytes
	(*vec).array[initCapacity];
}
...
//
// get size O(1)
int vecGetSize(struct Vector *vec) {
	return (*vec).size;
}
//// check empty O(1)
...
void truncate(struct Vector *vec, int size) {
	(*vec).capacity = size;
}
...
int main() {
	...
	printf("Get size: %d\n", vecGetSize(&vec));
	// Truncate:
	truncate(&vec, vecGetSize(&vec));
	printf("Da cut dung luong thua! %d", vec.capacity);
	// Print:
